...
void KeyboardInfo::parseKeyboardInfoFile(std::string_view file)
{
	...
	ranges::sort(msxKeyNames, StringOp::caseless{}, &MsxKeyName::name);
}
...
{
	return to_vector<std::string_view>(view::transform(msxKeyNames, &MsxKeyName::name));
}
